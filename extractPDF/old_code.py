...
def extract_pdf(file, path_root_output):
    ...
    # print(f"answer_options: {answers_options}")
    # print(f"len answer_options: {len(answers_options)}")
...
def get_base64_title(page, coors):
    ...
    if crop_box.is_empty == False and crop_box.is_infinite == False:
    ...
    for block in blocks:
        if "lines" in block:
            ...
            if len(block['lines']) >= 2:
                ...
                # print(f"text_span_1 {text_span_1} vs text_span_2 {text_span_2}")
                # Check various answer formats
                if check_correct_answer_type_1(text_span_1):
                    # print(f"type 1 {get_correct_answer_type_1(blocks)}")
                    return get_correct_answer_type_1(blocks)
                elif check_answer_option_title(text_span_2):
                    # print(f"type 2 {get_correct_answer_type_2(blocks)}")
...
def check_correct_answer_type_1(text_span):
    ...
    Returns:
        bool: True if matches answer type format
    """
    # Allow formats like "1. A", "1. Đ S", "1. T F", "1.2", "1. hello", "1. abc c", "1. -1.16"
...
def check_answer_option_title(text_span):
    ...
    Returns:
        bool: True if single answer option
    """
    # if re.search(r"^[A-FĐSTF]$", text_span):
    #     return True
    # return False
    return True
...
def check_correct_answer_type_special(text_span):
    ...
    Returns:
        bool: True if matches special formats
    """
    # Match answers like "Đ S S Đ", "T F", numbers, or text
            ...
            for line in block['lines']:
                ...
                if check_correct_answer_type_1(text_spans):
                    ...
                    if match:
                        ...
                        # print(f"arr_correct ['{question_number}', '{answer_text}']")
